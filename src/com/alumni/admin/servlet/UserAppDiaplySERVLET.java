		...
		try {
			...
			if (bean == null) {
				...
				response.getWriter().write(
						"<table class='ma'  border='0'  cellpadding='10' cellspacing='10'><tr><th>No.</th><th>User Name</th><th>Enroll No.</th><th>Subject</th><th>Message</th><th>Status</th><th>Submit</th></tr>");
				for (UserAppBEAN b : bean) {
					...
					int g = b.getStatus();
					int h = b.getDoc_Id();
					response.getWriter().write("<tr><td>" + n + "</td><td>" + a + "</td><td>" + c + "</td><td>" + d
							+ "</td><td>" + e + "</td><td><select name=status>");
					if (g == 1) {
					...
					response.getWriter().write("</select></td><td><a href='../UpdateUserAppSERVLET?result=" + h
							+ "'><button type=button>Submit</button></a></td></tr>");
					n++;
				}
