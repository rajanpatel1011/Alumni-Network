		...
		if (bean1 == null) {
			...
			try {
				...
				if (branchlist == null)
					...
					int n = 1;
					response.getWriter()
							.write("<table width='100%'><tr><th>No.</th><th>Branch</th><th>Delete</th></tr>");
					for (BranchDrpBEAN bean : branchlist) {
						String brancharray = bean.getBranch();
						int id = bean.getBranchId();
						response.getWriter()
								.write("<tr><td>" + n + "</td><td>" + brancharray
										+ "</td><td><a href='../DelBranchSERVLET?flag=" + id
										+ "'><button type=button id=del >Delete</button></a> </td></tr>");
						n++;
					}
