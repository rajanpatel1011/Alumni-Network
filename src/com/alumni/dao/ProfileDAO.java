...
public class ProfileDAO {
	...
	public ProfileBEAN fetch(int u_Id) throws SQLException {
		...
		try {
			...
			PreparedStatement st = con.prepareStatement(
					"select m_Id,u_Id,pwd,f_name,m_name,l_name,gender,dob,Mob_No,email_Id,hobbies,add1,add2,tot_exp_month,tot_exp_year,keyskills,hobbies,profile_pic from members where m_id=? ");
			st.setInt(1, u_Id);
	...
	public OtherEducationBean othereducationfatch(int mid) throws SQLException {
		...
		try {
			cn = ConnectionDAO.getConnection();
			PreparedStatement ps = cn.prepareStatement(
					"SELECT degree_type.degree_Type,cource_name,year,college_name,e_Id FROM edu_details,degree_type,members WHERE (edu_details.d_Id=degree_type.d_Id)AND(members.m_Id=edu_details.m_Id) AND (degree_type.degree_Type='other')AND(edu_details.m_Id=?);");
			ps.setInt(1, mid);
			ResultSet rs = ps.executeQuery();
			if (rs.next()) {
				do {
	...
	public List<JobBean> fatchMemJob(int m_Id) throws SQLException {
		...
		try {
			...
			PreparedStatement memjob = connectionhobbies.prepareStatement(
					"SELECT designation,start_month,start_year,end_month,end_year,industry_name,fun_Area,pr_Id FROM heroku_9915491d71b4e71.proff_details WHERE m_Id=?");
	...
	public void updateprofile(ProfileBEAN bean) throws SQLException {
		...
		try {
			...
			PreparedStatement updatestatement = updateconnection.prepareStatement(
					"UPDATE members SET pwd=?,f_name=?,m_name=?,l_name=?,email_Id=?,Mob_No=?,gender=?,add1=?,add2=?,tot_exp_month=?,tot_exp_year=?,keyskills=?,hobbies=?,profile_pic=? WHERE m_Id=?");
	...
	public void updateeducationdeatil(EducationBEAN ed) throws SQLException {
		...
		try {
			...
			if (rs.next()) {
				...
				PreparedStatement highestdegree = con3
						.prepareStatement("UPDATE members SET highest_degree=? WHERE m_Id=?;");
			...
			PreparedStatement quaryforupdateeducation = con
					.prepareStatement("UPDATE edu_details SET d_Id=?,college_name=? where e_Id=?");
...
//			String degree=ed.getDegree();
			...
			con = ConnectionDAO.getConnection();
			PreparedStatement quaryforupdateothereducation = con
					.prepareStatement("UPDATE edu_details SET cource_name=?,year=?,college_name=? where e_Id=?");
...
//------------------- update jobs details in memeber profile---------------
		...
		try {
			...
			for (JobBean list : listofjobs) {
				...
				PreparedStatement pr = con.prepareStatement(
						"UPDATE proff_Details SET designation=?,fun_Area=?,start_month=?,start_year=?,end_month=?,end_year=?,industry_name=? where pr_Id=?;");
	...
	public List<EducationBEAN> getbranchlist(int u_Id) throws SQLException {
		...
		try {
			con = ConnectionDAO.getConnection();
			PreparedStatement branch = con.prepareStatement(
					"SELECT branch.b_Id,Branch_Name FROM branch,members where (m_Id=?) and (branch.b_Id<>members.b_Id); ");
	...
	public List<EducationBEAN> getdegreelist(int u_Id) throws SQLException {
		...
		try {
			con = ConnectionDAO.getConnection();
			PreparedStatement degree = con.prepareStatement(
					"SELECT d_Id,degree_Type FROM degree_type,members where (degree_Type<>'other') AND(degree_Type<>'---')AND (degree_type.degree_Type<>members.highest_degree)AND(m_Id=?); ");
...
//------------------------------------ add job ---------------------------------------
	public int addjob(JobBean bean) throws SQLException {
		...
		try {
			con = ConnectionDAO.getConnection();
			PreparedStatement degree = con.prepareStatement("INSERT INTO proff_details(m_Id) VALUES (?);",
